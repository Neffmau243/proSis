...
> MySQL local `sistema_salud_ipress`; el frontend no usa ni debe crear otra
...
Las rutas sin ese prefijo, como `/pacientes/12`, son la navegación propuesta
para la aplicación web: todavía no existe un frontend dentro de este
repositorio, por lo que el equipo puede adaptar sus nombres sin cambiar el
contrato HTTP.

```text
Navegador → ruta de página propuesta → cliente HTTP → /api/v1 → FastAPI → MySQL
```
...
| --- | :---: | :---: | --- |
| Pacientes: consultar, crear, editar, responsables y riesgos | Sí | Sí, con ámbito clínico | El profesional solo ve pacientes de sus establecimientos asignados. |
...
| Ruta web propuesta | Vista / ventana | Lecturas | Escrituras | Roles |
| --- | --- | --- | --- | --- |
...
| `/pacientes` | Tabla/buscador de pacientes | `GET /patients` | — | ADMIN, PROFESIONAL |
...
| `/pacientes/:patientId` | Ficha del paciente con pestañas Datos, Responsables, Riesgos y Atenciones | `GET /patients/{patientId}`; `GET /atenciones?paciente_id={patientId}` | Según pestaña | ADMIN, PROFESIONAL |
| `/pacientes/:patientId/editar` | Edición de datos base | `GET /patients/{patientId}` y catálogos | `PATCH /patients/{patientId}` | ADMIN, PROFESIONAL |
...
| Ruta web propuesta | Vista / ventana | Lecturas | Escrituras |
| --- | --- | --- | --- |
| `/atenciones/nueva?pacienteId=:id` | Formulario clínico por pasos | Paciente, catálogos y consultorios/profesionales del establecimiento | `POST /atenciones` |
...
`grupo_etario_codigo`, edad ni estado: el backend los calcula/controla.
...
Antes de enviar una atención, el selector de establecimiento, consultorio y
profesional debe reflejar una asignación vigente. Aun así, no confiar solo en
la UI: el backend rechazará relaciones que no pertenezcan al profesional
autenticado. Tras emitir un documento, actualizar la ficha local y desactivar
la opción de anulación mientras no existan endpoints de anulación/cierre de
documentos.

## 7. Inventario completo de API por módulo

Esta lista complementa las páginas anteriores y permite al frontend comparar
su cliente HTTP con las 54 operaciones disponibles.
...
| Atenciones | `GET`, `POST /atenciones`; `GET /atenciones/busqueda`; `GET /atenciones/{attentionId}`; `POST /atenciones/{attentionId}/anulacion`. |
...
## 8. Vacíos actuales que frontend debe respetar
...
## 9. Lista de verificación antes de conectar el frontend
